...
- **Feature:** GROUP DEFINITION round-trip — captures raw block text (group names + member ranges) from both standalone `GROUP` blocks and `START`-wrapped blocks. Includes `END` marker in stored text for valid STAAD output. Written back in export.
