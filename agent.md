...
- ✅ **Node support markers** — fixed = red flat rectangular plate, pinned = yellow cone, custom fixity (e.g. `FIXED BUT MZ KFY 30`) = green cone + small text chip listing the releases (`MZ KFY 30` — free DOF tokens + spring values). Explicitly-free nodes show no marker. Spring supports keep the teal diamond. Text built by `nodeFixityReleaseText()` in `types.ts`. Chips can be hidden via the toolbar **Fixity text** toggle (independent of the Labels toggle; not persisted in project files).
- ✅ **Label text size** — toolbar **Text S/M/L** segmented control scales ALL label overlays (node, member, fixity chips, load-value labels) via a `--label-scale` CSS variable on the label layer (`LABEL_TEXT_SCALES` in `types.ts`: 0.8/1/1.3). One setter in `Labels.setTextScale()`; not persisted.
...
- **Fix:** `PRISMATIC` members silently dropped on import — `parseMemberProperty()` had no `PRISMATIC` branch, so `section`/`sectionProps` stayed `undefined` and the writer's `if (!m.section) continue` then omitted the whole `MEMBER PROPERTY` block on export. Fixed by adding the `PRISMATIC` parser branch (params kept verbatim, shape `other`), adding `"PRISMATIC"` to `SECTION_STAAD_KEYWORDS` + `staadKeywordToShape()`, and widening the writer guard to `m.section || m.sectionStaadKeyword`. Verified against `STAAD/P103/Movable Platform/Movable_Platform.txt` (7 members round-trip stable).
- **Lesson:** a new STAAD section keyword must be added in three places together — parser branch, `SECTION_STAAD_KEYWORDS` + `staadKeywordToShape()`, writer `hasSection`/`continue` guard — otherwise the loss is silent (parse "succeeds", export just omits the block). Always round-trip check (`parseStd` → `writeStd` → re-parse) when touching section handling.
- **Record:** canonical `PRISMATIC` hollow-section form (Movable_Platform, 2026-09-13) combines explicit properties with dims — e.g. `1 TO 6 PRISMATIC AX 0.004224 IZ 5.553E-6 IY 5.553E-6 IX 8.177E-6 YD 0.100 ZD 0.100` (100x100x12 SHS) and `7 PRISMATIC AX 0.004211 IZ 3.909E-6 IY 1.031E-6 IX 2.151E-6 YD 0.100 ZD 0.050` (100x50 RHS). Long lines use trailing `-` continuation (already joined in `preprocess`); the writer now applies `wrapLine()` to `MEMBER PROPERTY` lines so exports stay within `INPUT WIDTH 79`, and the empty-props `PRISMATIC` fallback uses the SHS form (never `PRIS`, which would silently change the section type).
