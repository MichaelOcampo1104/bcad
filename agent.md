...
- ✅ **Node support markers** — fixed = red flat rectangular plate, pinned = yellow cone, custom fixity (e.g. `FIXED BUT MZ KFY 30`) = green cone + small text chip listing the releases (`MZ KFY 30` — free DOF tokens + spring values). Explicitly-free nodes show no marker. Spring supports keep the teal diamond. Text built by `nodeFixityReleaseText()` in `types.ts`.
